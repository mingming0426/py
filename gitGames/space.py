...
class Game():
    def __init__(
            self):  # All class will have a method __init__ which provide instructions on what arguments is needed, example below at *
...
class Pen(turtle.Turtle):  # Example of class inheritance, inheriting from the parent 'turtle.Turtle'
    def __init__(
            self):  # The 'child' class (PEN)  inherit the properties and methods from the Parent class (turtle.Turtle)
        turtle.Turtle.__init__(
            self)  # This line of code is needed to keep the inheritance of the parent's __init__() function otherwise
